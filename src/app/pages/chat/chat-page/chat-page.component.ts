...
export class ChatPageComponent implements OnInit, AfterViewChecked {
  ...
  ngOnInit(): void {
    ...
    this.onlineUsers = users.filter((user) => user.isOnline && user.id !== this.generalUser.id);

    // Add all online users as separate chat tabs if not already present
    this.onlineUsers.forEach(user => {
      const existingTab = this.chatTabs.find(tab => tab.id === user.id.toString());
      if (!existingTab) {
    ...
    // Ensure 'general' tab exists
    const generalTab = this.chatTabs.find(tab => tab.id === 'general');
    if (!generalTab) {
      this.chatService.addChatTab({
        id: 'general',
        label: 'General',
